...
class HomeDbRepoImpl extends DatabaseAccessor<MaxiPocketDatabase>
    with _$HomeDbRepoImplMixin
    implements HomeDbRepo {
  final HomeDbSource _homeDbSource;

  HomeDbRepoImpl(super.database, this._homeDbSource);

  /// Fetches all subscription records from the database and maps them to domain entities.
  @override
  Future<List<SubscriptionEntity>> getSubscriptionsData() async {
    final List<SubscriptionDto> subscriptions = await _homeDbSource
  ...
  /// Fetches all financing records from the database and maps them to domain entities.
  @override
  Future<List<FinancingEntity>> getFinancingsData() async {
    final List<FinancingDto> financings = await _homeDbSource
        .getFinancingsData();

    final List<FinancingEntity> financingEntities = financings
        .map((FinancingDto dto) => dto.toEntity())
        .toList();
