...
class ExpensesDbSourceImpl extends DatabaseAccessor<MaxiPocketDatabase>
    ...
    await transaction(() async {
      switch (object) {
        ...
        case final AppointmentDto dto:
          ...
          await into(
            appointmentsTable,
          ).insert(AppointmentsTableCompanion.insert(location: dto.location, foreignId: BigInt.from(insertedCommonId)));
        default:
  ...
  Future<void> _logInsertedRow(ExpenseDbInsertDto object, int commonId) async {
    try {
      ...
      final CommonData common = await (select(
        commonDataTable,
      )..where(($CommonDataTableTable t) => t.primaryId.equals(id))).getSingle();

      switch (object) {
        case SubscriptionDto _:
          final Subscriptions sub = await (select(
            subscriptionsTable,
          )..where(($SubscriptionsTableTable t) => t.foreignId.equals(id))).getSingle();
          _logger.d('[insertQuery] Subscription — common: $common, subscription: $sub');
        case FinancingDto _:
          final Financing fin = await (select(
            financingTable,
          )..where(($FinancingTableTable t) => t.foreignId.equals(id))).getSingle();
          _logger.d('[insertQuery] Financing — common: $common, financing: $fin');
        case AppointmentDto _:
          final Appointment apt = await (select(
            appointmentsTable,
          )..where(($AppointmentsTableTable t) => t.foreignId.equals(id))).getSingle();
          _logger.d('[insertQuery] Appointment — common: $common, appointment: $apt');
        default:
          break;
      }
    } catch (e, st) {
