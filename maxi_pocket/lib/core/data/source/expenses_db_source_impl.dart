...
import 'package:maxi_pocket/core/data/repo/source/dto/expense_db_dto.dart' show ExpenseDbDto, ExpenseDbInsertDto;
...
@DriftAccessor(tables: <Type>[CommonDataTable, SubscriptionsTable, FinancingTable, AppointmentsTable])
class ExpensesDbSourceImpl extends DatabaseAccessor<MaxiPocketDatabase>
        ...
        case final SubscriptionDto dto:
          ...
          await into(subscriptionsTable).insert(
            SubscriptionsTableCompanion.insert(
              ...
              nextPaymentDate: Value<DateTime>(dto.expense.eventDate.nextPaymentDate(dto.frequency)),
  ...
  @override
  Future<void> deleteQuery({required BigInt commonId, required MaxiPocketExpensesType type}) => transaction(() async {
    switch (type) {
      case MaxiPocketExpensesType.subscription:
        await (delete(subscriptionsTable)..where(($SubscriptionsTableTable t) => t.foreignId.equals(commonId))).go();
      case MaxiPocketExpensesType.financing:
        await (delete(financingTable)..where(($FinancingTableTable t) => t.foreignId.equals(commonId))).go();
      case MaxiPocketExpensesType.appointments:
        await (delete(appointmentsTable)..where(($AppointmentsTableTable t) => t.foreignId.equals(commonId))).go();
      default:
        throw ArgumentError('Unsupported expense type: $type');
    }
    await (delete(commonDataTable)..where(($CommonDataTableTable t) => t.primaryId.equals(commonId))).go();
  });

  /// Updates the common row and the type-specific row for [commonId] within a transaction.
  @override
  Future<void> updateQuery({required BigInt commonId, required ExpenseDbInsertDto object}) => transaction(() async {
    switch (object) {
      case final SubscriptionDto dto:
        await (update(commonDataTable)..where(($CommonDataTableTable t) => t.primaryId.equals(commonId))).write(
        ...
        await (update(subscriptionsTable)..where(($SubscriptionsTableTable t) => t.foreignId.equals(commonId))).write(
      ...
      case final FinancingDto dto:
        ...
        await (update(financingTable)..where(($FinancingTableTable t) => t.foreignId.equals(commonId))).write(
          FinancingTableCompanion(
            ...
            nextPaymentDate: Value<DateTime>(
              dto.nextPaymentDate ?? dto.expense.eventDate.nextPaymentDate(MaxiPocketExpensesFrequency.monthly),
            ),
          ),
        );
      case final AppointmentDto dto:
        ...
        await (update(appointmentsTable)..where(($AppointmentsTableTable t) => t.foreignId.equals(commonId))).write(
          AppointmentsTableCompanion(location: Value<String>(dto.location), hour: Value<String>(dto.hour)),
        );
      default:
        break;
    }
  ...
  ///
  /// Logs only ids and type, not field values, since expense records can hold
  /// personal financial data (amounts, names, locations).
  Future<void> _logInsertedRow(ExpenseDbInsertDto object, int commonId) async {
    try {
      final BigInt id = BigInt.from(commonId);
      await (select(commonDataTable)..where(($CommonDataTableTable t) => t.primaryId.equals(id))).getSingle();

      switch (object) {
        case SubscriptionDto _:
          await (select(subscriptionsTable)..where(($SubscriptionsTableTable t) => t.foreignId.equals(id))).getSingle();
          _logger.d('[insertQuery] Subscription inserted — commonId: $commonId');
        case FinancingDto _:
          await (select(financingTable)..where(($FinancingTableTable t) => t.foreignId.equals(id))).getSingle();
          _logger.d('[insertQuery] Financing inserted — commonId: $commonId');
        case AppointmentDto _:
          await (select(appointmentsTable)..where(($AppointmentsTableTable t) => t.foreignId.equals(id))).getSingle();
          _logger.d('[insertQuery] Appointment inserted — commonId: $commonId');
        default:
          break;
      }
    } catch (e, st) {
      _logger.e('[_logInsertedRow] Failed to read back inserted row', error: e, stackTrace: st);
    }
  }

  /// Writes shared expense fields to [CommonDataTable] and returns the generated primary key.
  Future<int> _insertCommonData(ExpenseDbDto expense) => into(commonDataTable).insert(
    CommonDataTableCompanion.insert(
      ...
      updatedAt: Value<DateTime>(DateTime.now()),
      localeTimezone: expense.localeTimezone.toString(),
    ),
  );
}
